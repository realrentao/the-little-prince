...
(function () {
  ...
  function openChapter(i, keepScroll) {
    ...
    if (headerIll) {
      const fig = illustImg(headerIll.file, ch.title_es + " 原版插画", true);
    ...
    ch.paras.forEach((p, pi) => {
      p.sents.forEach((s, si) => {
        ...
        esLine.appendChild(renderEs(s.es));
        ...
        if (illustList && illustList.length) {
          illustList.forEach((it) => {
            const fig = illustImg(it.file, ch.title_es + " 插画", false);
  ...
  */
  ...
  function updateNow(pid) {
    ...
    $("nowTitle").textContent = ch ? `${ch.title_es} · ${ch.title_zh}` : "";
    let sub;
