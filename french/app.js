...
(function () {
  ...
  function openChapter(i, keepScroll) {
    ...
    if (headerIll) {
      const fig = illustImg(headerIll.file, ch.title_fr + " 原版插画", true);
    ...
    ch.paras.forEach((p, pi) => {
      p.sents.forEach((s, si) => {
        ...
        esLine.appendChild(renderFr(s.fr));
        ...
        if (illustList && illustList.length) {
          illustList.forEach((it) => {
            const fig = illustImg(it.file, ch.title_fr + " 插画", false);
  ...
  */
  ...
  function updateNow(pid) {
    ...
    $("nowTitle").textContent = ch ? `${ch.title_fr} · ${ch.title_zh}` : "";
    let sub;
